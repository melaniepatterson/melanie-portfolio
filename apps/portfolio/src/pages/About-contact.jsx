...
// Bottom-edge decorative accent — static, no motion, on both desktop and
// mobile. Position/size come from the caller (desktop pokes past the
// bottom edge; mobile sits lower and pokes past the right edge instead).
// Same z-index reasoning as StarLeft above — negative, not positive, since
// this renders as a child of .page rather than a true sibling of it.
...
export default function AboutContact() {
  ...
  // StarLeft's cursor-repulsion motion — desktop only (no real cursor on
  // touch), same physics as Home's stars (Radialgradient.jsx), just
  // driving one element instead of four. Both stars render on mobile too,
  // just without this — static, repositioned per the isMobile styles below.
  useEffect(() => {
    if (isMobile) return;
    const handleMouseMove = (e) => {
      const el = starLeftRef.current;
      if (!el) return;
      const style = computeRepulsion(el.getBoundingClientRect(), e.clientX, e.clientY, 150, 45, 5);
      applyRepulsionStyle(el, style);
    };
